  ...
  return (
    <article className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm transition hover:border-zinc-300">
      ...
      <h2 className="mt-2 text-2xl font-semibold tracking-tight text-zinc-900">
        <Link className="underline-offset-4 hover:underline" href={`/posts/${slug}`}>
          {title}
        </Link>
      </h2>
      <p className="mt-3 text-base text-zinc-700">{excerpt}</p>
      <Link className="mt-4 inline-block text-sm font-medium text-zinc-900 underline-offset-4 hover:underline" href={`/posts/${slug}`}>
        Read post
      </Link>
    </article>
  );
